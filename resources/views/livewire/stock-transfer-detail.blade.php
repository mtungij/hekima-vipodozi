<div>
    <x-slot name="header">
        <div class="flex justify-end gap-4">
            <a href="{{ route('stock-transfer.transfered')}}">
    ...
    <div class="max-w-2xl p-4 mx-auto bg-gray-200 print:bg-gray-100 my-8">
        ...
        <p class="text-lg uppercase text-center text-gray-900 bg-blue-400">
            Products from - {{ $stockTransfer?->branch?->name }} - to - {{ $stockTransfer?->toBranch?->name}}
        </p>
        ...
        <div class="py-4 italic text-center text-gray-600">
            <p>{{ date('d/m/Y H:m', strtotime($stockTransfer->created_at))}}</p>
            <p class="">Issued by <b>{{ $stockTransfer?->user?->name }}</b></p>
        </div>
    </div>
