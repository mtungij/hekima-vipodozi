<div class="space-y-6">
    <button
         x-data=""
         x-on:click.prevent="$dispatch('open-modal', 'view-product-modal{{ $form->product?->id }}')"
    >
        <x-heroicon-m-eye class="size-5 text-teal-500" />
    </button>

    <x-modal name="view-product-modal{{ $form->product?->id }}" :show="$errors->isNotEmpty()" maxWidth="4xl" focusable>
        <form wire:submit="store" class="p-3 lg:p-6">
            ...
            <p class="mt-1 text-sm text-orange-500">
                {{ __("You're viewing products for the branch:") }} <span class="uppercase font-bold text-green-400">{{ auth()->user()?->branch?->name}}</span>
            </p>
