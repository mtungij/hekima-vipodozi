<div class="space-y-6">
    <x-primary-button
         x-data=""
         x-on:click.prevent="$dispatch('open-modal', 'create-product-modal')"
    >
        <x-heroicon-o-plus class="size-5" />
        <span class="hidden lg:inline-block">{{ __('Create product')}}</span>
    </x-primary-button>

    <x-modal name="create-product-modal" :show="$errors->isNotEmpty()" maxWidth="4xl" focusable>
        <form wire:submit="store" class="p-3 lg:p-6">
            ...
            <p class="mt-1 text-sm text-orange-500">
                {{ __("You're creating products for the branch:") }} <span class="uppercase font-bold text-green-400">{{ auth()->user()?->branch?->name}}</span>
            </p>
